import cv2 as cv
import numpy as np


def threshold_demo(image):
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    ret, binary = cv.threshold(gray, 127, 255, cv.THRESH_BINARY|cv.THRESH_OTSU)
    print("threshold value %s"%ret)
    cv.imshow("threshold_demo", binary)


def local_threshold(image, index):
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    binary = cv.adaptiveThreshold(gray, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 25, 10)
    cv.imshow("local_threshold_%s" %index, binary)


def custom_threshold(image,index):
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    h, w = gray.shape[:2]
    m = np.reshape(gray, [1, w*h])
    print("m.sum = %s" %(m.sum()))
    mean = m.sum() / (w*h)
    print("mean : ", mean)
    ret, binary = cv.threshold(gray, mean, 255, cv.THRESH_BINARY)
    cv.imshow("custom_threshold_%s" %index, binary)


print("--------- Python OpenCV Tutorial ---------")
for i in range(0, 12):
    src = cv.imread('./images/PATI01/%s.BMP' %i)
    # cv.namedWindow("input image", cv.WINDOW_AUTOSIZE)
    # cv.imshow("input image", src)
    # threshold_demo(src)
    # local_threshold(src, i)
    custom_threshold(src, i)
cv.waitKey(0)

cv.destroyAllWindows()
